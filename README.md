...
It starts the target app inside `tmux`, opens a detached WezTerm window for real rendering, captures a real PNG with macOS `screencapture`, and returns text plus the local PNG file path through MCP tools.
...
2. Call `tui_snapshot` and inspect `textView` plus the local PNG path in `visual.imageArtifactId`.

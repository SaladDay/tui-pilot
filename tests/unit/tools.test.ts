...
const mocks = vi.hoisted(() => ({
  ...
  discoverWeztermWindowMock: vi.fn(async () => ({
    windowId: 789,
    pid: 4242,
    bounds: {
      x: 10,
  ...
  launchWeztermMock: vi.fn<() => { pid: number | undefined }>(() => ({ pid: 4242 })),
}))
...
vi.mock('../../src/controllers/macos-window.js', () => ({
  ...
  discoverWeztermWindow: mocks.discoverWeztermWindowMock,
  captureWindow: mocks.captureWindowMock,
}))

vi.mock('../../src/controllers/wezterm.js', () => ({
  launchWezterm: mocks.launchWeztermMock,
}))
...
describe('buildToolList', () => {
  beforeEach(() => {
    ...
    mocks.discoverWeztermWindowMock.mockReset()
    mocks.discoverWeztermWindowMock.mockResolvedValue({
      windowId: 789,
      pid: 4242,
      bounds: {
        x: 10,
    ...
    mocks.launchWeztermMock.mockReset()
    mocks.launchWeztermMock.mockReturnValue({ pid: 4242 })
  })
  ...
  it('starts a session with the requested dimensions and stores discovered window metadata', async () => {
    ...
    expect(mocks.discoverWeztermWindowMock).toHaveBeenCalledWith({ pid: 4242 })
  ...
  it('stores the discovered terminal pid as the canonical session pid', async () => {
    ...
    mocks.launchWeztermMock.mockReturnValue({ pid: 4242 })
    mocks.discoverWeztermWindowMock
    ...
    expect(mocks.discoverWeztermWindowMock).toHaveBeenNthCalledWith(1, { pid: 4242 })
    expect(mocks.discoverWeztermWindowMock).toHaveBeenNthCalledWith(2, { pid: 5252 })
  })
  ...
  it('cleans up the tmux session when startup fails after the session launches', async () => {
    ...
    mocks.discoverWeztermWindowMock.mockRejectedValue(new Error('window not found'))
  ...
  it('fails fast when wezterm launch does not provide a pid', async () => {
    const tools = await importTools()
    const startTool = getTool(tools, 'tui_start')

    mocks.launchWeztermMock.mockReturnValue({ pid: undefined })
    ...
    await expect(startTool!.handler({
      ...
      cols: 132,
      rows: 36,
    })).rejects.toThrow('wezterm launch did not provide a pid')

    expect(mocks.buildWindowHelperMock).not.toHaveBeenCalled()
    expect(mocks.discoverWeztermWindowMock).not.toHaveBeenCalled()
    expect(mocks.killSessionMock).toHaveBeenCalledWith('tui-pilot-session-123')
  })
  ...
  it('retries window discovery across a short startup polling window before succeeding', async () => {
    ...
    for (let attempt = 0; attempt < 6; attempt += 1) {
      mocks.discoverWeztermWindowMock.mockRejectedValueOnce(new Error(`not ready ${attempt + 1}`))
    }

    mocks.discoverWeztermWindowMock.mockResolvedValueOnce({
      windowId: 790,
      pid: 4242,
      bounds: {
        x: 30,
    ...
    expect(mocks.discoverWeztermWindowMock).toHaveBeenCalledTimes(7)
  ...
  it('does not let an older snapshot completion overwrite newer window metadata', async () => {
    ...
    expect(mocks.discoverWeztermWindowMock).toHaveBeenNthCalledWith(1, { pid: 4242 })
    expect(mocks.discoverWeztermWindowMock).toHaveBeenNthCalledWith(2, { pid: 4242 })
    expect(mocks.discoverWeztermWindowMock).toHaveBeenNthCalledWith(3, { pid: 6100 })
  })
  ...
  it('refreshes window metadata before capturing a snapshot', async () => {
    ...
    expect(mocks.discoverWeztermWindowMock).toHaveBeenLastCalledWith({ pid: 4242 })
    expect(mocks.captureWindowMock).toHaveBeenCalledWith(
      790,
      expect.stringContaining('/.tui-pilot/artifacts/session-123/snapshot-1.png'),
    )
